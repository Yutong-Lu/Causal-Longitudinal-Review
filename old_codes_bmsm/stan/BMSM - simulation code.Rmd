...
## FREQUENTIST MSM
```{r, echo = FALSE}
Wmsm.out <- weightitMSM(list(z1 ~ x11d + x12d + x13 + x14 + x15,
                                 z2 ~ x21d + x22d + x23 + x24 + x25+ z1),
                            data = obs, method = "ps",
                            stabilize = TRUE)
    ...
    cum.fit <- svyglm(y ~ z1 + z2 + z1*z2 + x21d + x22d + x23 + x24 + x25, design = d.w.msm)
    
    summary(cum.fit)
    
y_00<-mean(predict(truemodel, newdata = cbind(z1 = 0, z2=0, (obs[,10:14]))))
y_11<-mean(predict(truemodel, newdata = cbind(z1 = 1, z2=1, (obs[,10:14]))))
```
...
```{r, echo = FALSE}
    ...
    obs <- obs %>% dplyr::select(x11d, x12d, x13, x14, x15, x21d, x22d, x23, x24, x25, z1, z2, y)
    # ltmle with superlearner + kitchen sink gform + Qform;
    tmle_model_s <- ltmle(obs,
                          Anodes = c ("z1","z2") ,
                          Lnodes = c ("x11d", "x12d","x13", "x14", "x15", "x21d", "x22d", "x23", "x24", "x25"), 
                          Ynodes = c("y"), 
                          survivalOutcome =FALSE,
                          Qform = c(
                            x21d = "Q.kplus1 ~ x11d + x12d + x13 + x14 + x15 + z1",
                                     y = "Q.kplus1 ~ x21d + x22d + x23 + x24 + x25 + z1 + z2 + z1*z2"),
                          gform = c("z1 ~ x11d + x12d + x13 + x14 + x15",
                                    "z2 ~ x21d + x22d + x23 + x24 + x25 + z1"),
...
x_2 <- rnorm(ntot, mean = x_1 + z_1*0.2, sd = 0.1)
...
obslong2 <- data.frame(x = c(x_1, x_2), z = c(z_1, z_2), y = c(rep(NA, ntot), yobs))
...
y <- z_2*0.60 + z_1 * 0.36 + z_1*z_2*(-0.3) + x_2*0.3 + 2 ## TRUE EFFECT IS 0.66
yobs <- rnorm(ntot, mean = y, sd = 0.4)
obswide2 <- data.frame(x_1, x_2, z_1, z_2, yobs)


Wmsm.out <- weightitMSM(list(z_1 ~ x_1,
                                 z_2 ~ x_1 + x_2 + z_1),
                            data = obswide2, method = "ps",
                            stabilize = TRUE)
    ...
    cum.fit <- svyglm(yobs ~ z_1 + z_2 + z_1*z_2 + x_2, design = d.w.msm)
...
```{r, echo = FALSE}
    ...
    # tmle usign ltmle packages;
    obswide2 <- obswide2 %>% dplyr::select(x11d, x12d, x13, x14, x15, x21d, x22d, x23, x24, x25, z1, z2, y)
    # ltmle with superlearner + kitchen sink gform + Qform;
    tmle_model_s <- ltmle(obswide2,
                          Anodes = c ("z_1","z_2") ,
                          Lnodes = c ("x_1", "x_2"), 
                          Ynodes = c("yobs"), 
                          survivalOutcome =FALSE,
                          Qform = c(
                            x_2 = "Q.kplus1 ~ x_1 + z_1",
                                     yobs = "Q.kplus1 ~ x_2 + z_1 + z_2 + z_1*z_2"),
                          gform = c("z_1 ~ x_1",
                                    "z_2 ~ x_2 + z_1"),
                          SL.library = "default", #with superlearner;
                          abar = list(c(1,1), c(0,0)),
