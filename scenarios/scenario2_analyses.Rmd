...
Analyzing simulated dataset for scenario 2
...
# Loading simulated dataset 2
```{r}
dat2 <- read_csv("datasets/scenario_2_wide_dataset.csv")
dat2_new <- read_csv("datasets/scenario_2_long_dataset.csv")
```
...
#                       list(c(threshold, 1, Inf))) #Kuan: what is this?;
...
# OR point estimate;
# > (0.5130911/(1-0.5130911))/(0.5471206/(1-0.5471206))
# [1] 0.8722606
```
...
#kuan can't have cum_a in the data;
dat2 <- dat2 %>% select(-cum_a)

tmle_model_noSL_ksG_nullQ <- ltmle(dat2,
                    Anodes = c ("a_1","a_2") ,
...
summary(tmle_model_noSL_ksG_nullQ, estimator="tmle")

# tmle with superlearner + kitchen sink gform + null Qform;
tmle_model_SL_ksG_nullQ <- ltmle(dat2,
                    Anodes = c ("a_1","a_2") ,
                    Lnodes = c ("L1_1", "L2_1", "L1_2", "L2_2"), 
                    Ynodes = c("y"), 
                    survivalOutcome =FALSE,
...
summary(tmle_model_SL_ksG_nullQ, estimator="tmle")

#tmle package for gcomputation + kitchen sink gform + null Qform;
tmle_model_gcomp_nullQ <- ltmle(dat2,
                    Anodes = c ("a_1","a_2") ,
                    Lnodes = c ("L1_1", "L2_1", "L1_2", "L2_2"), 
                    Ynodes = c("y"), 
                    survivalOutcome =FALSE,
...
summary(tmle_model_gcomp_nullQ)

#tmle package for gcomputation + kitchen sink gform + correct Qform;
tmle_model_gcomp_correctQ <- ltmle(dat2,
                    Anodes = c ("a_1","a_2") ,
                    Lnodes = c ("L1_1", "L2_1", "L1_2", "L2_2"), 
                    Ynodes = c("y"), 
                    survivalOutcome =FALSE,
...
summary(tmle_model_gcomp_correctQ)
```
...
summary(svyglm(y ~ as.factor(cum_a), design = ds, family = "binomial"))

# ds_incorrect <- svydesign(id=~1, weights = ~ incorrect_weights, data = dat2)
# summary(svyglm(y ~ as.factor(cum_a), design=ds_incorrect, family = "binomial"))
...
summary(svyglm(y ~ as.factor(cum_a), design=ds_package, family = "binomial"))
