...
# https://github.com/ehsanx/simMSM/blob/master/R/simmsm.R
for(i in 1:1000){
set.seed(123+i)
simmsm(subjects = 1000, tpoints=2, psi = log(0.8), n=1)
}
...
simmsm <- function(subjects = 2500, tpoints = 10, psi = .3, n = 5){
  ...
  for (select.seed.val  in 1:n){
    # seeds
    #gamma =parameters for L, covariates
    gamma0 = log(3/7)
    gamma1 = 2
    gamma2 = log(0.5)
    gamma3 = log(1.5)
    #alpha = parameters for A, treatment
    alpha0= log(2/7)
    alpha1=0.5
    alpha2 = 0.5
    alpha3 = log(4)
    
    cut=30; #potentally cut w.r.t time;
    lam=0.01 #scale parameter for weibull, parameters to simulate event time for the survival outcome
    shape=1 #shape parameter for weibull, parameters to simulate event time for the survival outcome
    ...
    for (id in 1:subjects){
      ...
      L = double(tpoints) #a vector for time-dependent covariates, if tpoints=2, that a vector of 2 elements;
      A = double(tpoints)
      Y = double(tpoints)
      Ym = double(tpoints) #?
      pA = double(tpoints)
      pL = double(tpoints) #p likely probabiliy
      chk = double(tpoints) #?
      ty = double(tpoints)
      ...
      logitA = alpha0 + alpha1 * L[1] #simulate visit 1 treatment using covariate at visit 1;
      pA[1] = 1/(1+(1/exp(logitA))) 
      A[1] = rbinom(1,1,pA[1])
      
      # baseHZ*exp(x*Z) logHR = b*x1+a*x2, psi is the log HR, exp(psi) is the HR;
      # generate Y'sm survival outcome, simulating from proportional hazard model;
      chk[1] = exp(psi*A[1])
      if (T0 > chk[1]) {
        Y[1]=0 #IT0 indicator;
        ty[1]=NA
        ...
        Y[1]=1 #event flag;
        ty[1] = T0*exp(-psi*A[1]) #time to event;
      }
      
      j = 1 #no censoring at visit 1;
      Cense[1] = 0
      Cens[1] = 0
      pcens[1] = 0
